struct VARProcess{TB<:AbstractMatrix, TI<:Union{AbstractVector,Nothing}}
    ...
    function VARProcess(B::AbstractMatrix{TF},
        ...
        if n % m == 0
            ...
            throw(ArgumentError("coefficient matrix of size $(size(B)) is not accepted"))
        end
    end
end
...
function simulate!(εs::AbstractArray, var::VARProcess, Y0::AbstractArray;
    ...
    for t in length(rlag0)+1:size(εs, 2)
        ...
        var(view(εs, :, t, :), reshape(view(εs, :, rlag, :), N*length(rlag), size(εs,3)))
    end
    return εs
end

function simulate!(εs::AbstractArray, var::VARProcess; nlag::Integer=arorder(var))
    0 < nlag <= arorder(var) || throw(ArgumentError(
        "nlag must be positive and no greater than $(arorder(r))"))
    N = size(var.B, 1)
    size(εs, 1) == N || throw(DimensionMismatch(
